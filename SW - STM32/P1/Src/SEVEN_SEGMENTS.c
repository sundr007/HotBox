...
void SEVEN_SEG_Main(void)
{
	...
	for(i=0; i<SEVEN_SEG_UNITS; i++)
	{
		...
		if(gs_SEVENSEG_info[i].EN_State == ENABLED)
		{
			...
			for (j=0; j<DIGITS; j++)
			{
				HAL_GPIO_WritePin(SEVEN_SEG_CfgParam[i].EN_GPIO[j], SEVEN_SEG_CfgParam[i].EN_PIN[j], 1);
			}
			...
			HAL_GPIO_WritePin(SEVEN_SEG_CfgParam[i].EN_GPIO[Digit_Index], SEVEN_SEG_CfgParam[i].EN_PIN[Digit_Index], 0);
			/* Increment The Index Of The Digit To Be Displayed In The Next Time.. */
			gs_SEVENSEG_info[i].Digit_Index++;
			if(gs_SEVENSEG_info[i].Digit_Index == DIGITS+0)
			{
